...
then
	...
	VRRP_VARIABLE+=$'\n\t'
	VRRP_VARIABLE+="$(echo -e "option tcplog" )"
	VRRP_VARIABLE+=$'\n\t'
	VRRP_VARIABLE+="$(echo -e "default_backend $KubeClusterName" )"
	VRRP_VARIABLE+=$'\n'
	VRRP_VARIABLE+="$(echo -e "backend $KubeClusterName" )"
	VRRP_VARIABLE+=$'\n\t'
	VRRP_VARIABLE+="$(echo -e 'option httpchk GET /healthz' )"
	VRRP_VARIABLE+=$'\n\t'
	VRRP_VARIABLE+="$(echo -e "http-check expect status 200" )"
	VRRP_VARIABLE+=$'\n\t'
	VRRP_VARIABLE+="$(echo -e "mode tcp" )"
	VRRP_VARIABLE+=$'\n\t'
	VRRP_VARIABLE+="$(echo -e "option ssl-hello-chk" )"
	VRRP_VARIABLE+=$'\n\t'
